...
export async function getContentList<T extends keyof EndPoints["gets"]>(
  ...
  switch (key) {
    case "blogs":
      return await (
        await fetch("http://localhost:4321/assets/mock/blogs-list.json")
      ).json();
    case "works":
      return await (
        await fetch("http://localhost:4321/assets/mock/works-list.json")
      ).json();
    default:
...
export async function getContentDetail<T extends keyof EndPoints["get"]>(
  ...
  switch (key) {
    case "blogs":
      return await (
        await fetch("http://localhost:4321/assets/mock/blogs-detail.json")
      ).json();
    case "works":
      return await (
        await fetch("http://localhost:4321/assets/mock/works-detail.json")
      ).json();
    default:
      throw new Error("Invalid key");
  }
